...
ITEM_LINE_RE = re.compile(
    r"(?:^|\s)(\d{1,3})[.):]?\s+([A-Za-z][A-Za-z \-/&'(),.]{2,60}?)(?=\s+\d{1,3}[.):]?\s+[A-Za-z]|\s*$)"
)
...
def extract_annexure_lists(path):
    ...
    with pdfplumber.open(path) as pdf:
        ...
        for page_no, page in enumerate(pdf.pages, start=1):
            ...
            # Last resort. Many insurers render these lists with no ruling
            # lines at all, so no table extractor sees them. Parse the raw
            # text as numbered items instead. Handles two items per line.
            if len(items) == found_before:
                for line in text.split("\n"):
                    for m in ITEM_LINE_RE.finditer(line):
                        name = m.group(2).strip(" .:-")
                        if len(name) < 3 or name.upper() in HEADER_CELLS:
                            continue
                        items.append({
                            "item_name": re.sub(r"\s+", " ", name),
                            "category": current[0],
