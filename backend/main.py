...
- NEVER ASSERT A FACT NOBODY HAS STATED. If the rejection letter and the claim details are both silent on something, you do not know it, and neither advocate saying it makes it so. Never write "because the condition was undeclared" when nothing says it was undeclared. This applies to every fact, always.
- THERE IS EXACTLY ONE SITUATION IN WHICH A SILENCE MAKES A CASE UNDECIDABLE, and it is narrow. A pre-existing disease exclusion carries two separate conditions: a waiting period of so many months, and — for coverage after that period has expired — that the disease was declared at the time of application and accepted. When the months of continuous coverage EXCEED the waiting period, the time condition is satisfied and the exclusion can only stand on the declaration condition. If nothing states whether the disease was declared and accepted, return insufficient_information and name that missing declaration in the explanation. That is the whole of it.
- EVERYWHERE ELSE, DECIDE ON WHAT YOU WERE GIVEN. Specifically:
  * If the months of coverage are FEWER than the waiting period, the exclusion applies on the wording and the timeline, and you decide. Do not abstain because the diagnosis date is unstated. Do not abstain because the letter does not separately prove the condition is pre-existing — the insurer has asserted that it is, the waiting period has not run, and that is enough to say whether the rejection is supported by the wording.
  * Do not abstain because an exception in the wording has not been ruled out. There is always one — an accident, ported coverage, a renewal — and treating an unraised exception as a reason to abstain would make every rejection undecidable.
  * Do not abstain because a figure needed to compute an exact payable amount is unstated, when the verdict is the same whatever that figure turns out to be. A cap of "25% of sum insured or Rs.40,000, whichever is lower" is at most Rs 40,000 however large the sum insured is.
